...
def get_api_answer(current_timestamp):
    ...
    if  response.status_code != 200:
        message = f'Неудовлетворительный статус ответа: {response.status_code}'
...
def parse_status(homework):
    ...
    if 'homework_name' and 'status' not in homework:
        message = 'Ключи homework_name и status отсутствуют'
...
def check_tokens():
    if not PRACTICUM_TOKEN:
        logging.critical("Отсутствует обязательная переменная окружения: 'PRACTICUM_TOKEN' Программа принудительно остановлена.")
        return False
    if not TELEGRAM_TOKEN:
        logging.critical("Отсутствует обязательная переменная окружения: 'TELEGRAM_TOKEN' Программа принудительно остановлена.")
        return False
    if not TELEGRAM_CHAT_ID:
        logging.critical("Отсутствует обязательная переменная окружения: 'TELEGRAM_CHAT_ID' Программа принудительно остановлена.")
        return False                
    return True
